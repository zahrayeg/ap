...
public class RestaurantService {
    ...
    public DataResult<List<RestaurantDTO>> getSellerRestaurants(UUID sellerId) {
        List<RestaurantDTO> dtos = restaurantDAO
                .findBySellerId(sellerId)
                .stream()
                .map(RestaurantDTO::fromEntity)
                .collect(Collectors.toList());

        return new DataResult<List<RestaurantDTO>>(200, dtos);
    }

    // GET /restaurants/{id}/item
    public DataResult<List<FoodDTO>> getItemsByRestaurant(
            UUID restaurantId, UUID sellerId
    ) {
        boolean owns = restaurantDAO
                .findBySellerId(sellerId)
                .stream()
                .anyMatch(r -> r.getId().equals(restaurantId));

        if (!owns) {
            return new DataResult<List<FoodDTO>>(403, Collections.emptyList());
        }

        List<FoodDTO> dtos = foodDAO
                .findByRestaurantId(restaurantId)
                .stream()
                .map(FoodDTO::fromEntity)
                .collect(Collectors.toList());

        return new DataResult<List<FoodDTO>>(200, dtos);
    }

    // GET /restaurants/{id}/menu
    public DataResult<List<MenuDTO>> getMenusByRestaurant(
            UUID restaurantId, UUID sellerId
    ) {
        boolean owns = restaurantDAO
                .findBySellerId(sellerId)
        ...
        if (!owns) {
            return new DataResult<List<MenuDTO>>(403, Collections.emptyList());
        }

        List<MenuDTO> dtos = menuDAO
                .findByRestaurantId(restaurantId)
                .stream()
                .map(MenuDTO::fromEntity)
                .collect(Collectors.toList());

        return new DataResult<List<MenuDTO>>(200, dtos);
    }

...
public class AdminHttpHandler implements HttpHandler {
    ...
    public void handle(HttpExchange exchange) throws IOException {
        ...
        if ("/admin/orders".equals(path) && "GET".equalsIgnoreCase(method)) {
    ...
    private void handleChangeUserStatus(HttpExchange exchange,
        ...
        // خواندن بدنه‌ی درخواست (JSON body)
        ...
        // 1. اگر JSON body موجود و غیر خالی است، از Gson بخوانیم
        ...
        // 2. در غیر این صورت (body خالی)، اگر ?approved=… در کوئری هست، از آن استفاده کنیم
        else {
            Map<String, String> qs = queryToMap(exchange.getRequestURI().getRawQuery());
            if (qs.containsKey("approved")) {
                boolean approved = Boolean.parseBoolean(qs.get("approved"));
                dto = new StatusDTO(approved);
            }
        }

        // 3. اگر همچنان dto برابر null است، خطای Missing body بدهیم
        if (dto == null) {
            sendError(exchange, 400, "Missing request body or approved query parameter");
            return;
        }

        // فراخوانی سرویس برای به‌روزرسانی وضعیت
